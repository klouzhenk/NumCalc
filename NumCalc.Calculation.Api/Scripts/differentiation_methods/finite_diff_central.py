...
def solve(expression: str, x_point: float, h: float, order: int) -> str:
    try:
        ...
        if order == 1:
            value = (f(x_point + h) - f(x_point - h)) / (2 * h)
            latex = r"f'(x) \approx \frac{f(x+h) - f(x-h)}{2h}"
            description = "Central difference — 1st derivative, accuracy O(h²)"
            label = "f'"
        else:
            value = (f(x_point + h) - 2 * f(x_point) + f(x_point - h)) / (h ** 2)
            latex = r"f''(x) \approx \frac{f(x+h) - 2f(x) + f(x-h)}{h^2}"
            description = "Central difference — 2nd derivative, accuracy O(h²)"
            label = "f''"
